...
class BlurEffectHelper(
    ...
    private var blurOutAnimationDuration: Int = 500
) {
    ...
    init {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S
            && rootView.allViews.toList().isNotEmpty()
            && canUseRenderEffect
        ) {
    ...
    fun show() {
        ...
        if (isBlurFadeInProgress) {
            ...
            Blurry.with(activity).radius(blurRadius).sampling(blurSampling)
                .color(Color.argb(darkOpacity, 0, 0, 0))
                .animate(blurInAnimationDuration).onto(rootView)
        }
    }
    ...
    private fun fadeOutBlurView(blurryView: View) {
        ...
        blurFadeOutAnimation?.addListener(
            onStart = {
